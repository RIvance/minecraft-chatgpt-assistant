...
abstract class AssistantModel(val logger: Logger) {
    ...
    fun getPlayerInfoPrompt(player: Player): String {
        return (
            "The player ${player.name.string} is playing Minecraft ${player.server?.serverVersion}. "
        )
    }

    protected abstract fun getResponse(player: Player, prompt: String, config: RequestConfig): String

    fun getCommand(player: Player, prompt: String, config: RequestConfig): String {
